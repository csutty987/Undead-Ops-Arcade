...
int	SFML_PlayableSpriteObject::setPistolReloadAnimation(std::string spriteSheetFilename, std::string spriteInfoFilename, float animationTime)
{
	m_pistolReloadAnimationID = addAnimation(spriteSheetFilename, spriteInfoFilename, animationTime);
	m_animatedSpriteList[m_pistolReloadAnimationID]->setLoop(false);
	m_animatedSpriteList[m_pistolReloadAnimationID]->setCanbeInterrupted(false);
	return m_pistolReloadAnimationID;
}
...
int	SFML_PlayableSpriteObject::setRifleReloadAnimation(std::string spriteSheetFilename, std::string spriteInfoFilename, float animationTime)
{
	m_rifleReloadAnimationID = addAnimation(spriteSheetFilename, spriteInfoFilename, animationTime);
	m_animatedSpriteList[m_rifleReloadAnimationID]->setLoop(false);
	m_animatedSpriteList[m_rifleReloadAnimationID]->setCanbeInterrupted(false);
	return m_rifleReloadAnimationID;
}
...
bool SFML_PlayableSpriteObject::toShootState()
{
	...
	if (m_currentAnimation->canbeInterrupted() || m_currentAnimation->isCompleted())
	{
		...
		case PISTOL:
		{
			...
			setCurrentAnimation(m_pistolReloadAnimationID);
		}
		...
		{
			m_playerAction = SHOOT;
			setCurrentAnimation(m_rifleReloadAnimationID);
		}
...
bool SFML_PlayableSpriteObject::toReloadState()
{
...
void SFML_PlayableSpriteObject::setDamage(int damage)
{
	m_damage = damage;
	pDamage = damage;
}

int SFML_PlayableSpriteObject::getDamage()
{
	return pDamage;
}

void SFML_PlayableSpriteObject::setHealth(int damage)
{
	m_health = m_health - damage;
}

int SFML_PlayableSpriteObject::getHealth()
{
	return pHealth;
}
